...
- `h_flex()` applies `items_center()` by default. Do not use it as a full-height split-pane root unless vertical centering is intended. For app panels that must stretch vertically, prefer `div().flex().flex_row().flex_1().min_h_0().overflow_hidden()`, and give child panels `h_full()` / `min_h_0()`.
- For scrollable panel bodies, use the same structure as `Dialog` / `Sheet`: an outer `div().flex_1().min_h_0().overflow_hidden()` and an inner `v_flex().size_full().overflow_y_scrollbar()`. Keep toolbars, headers, and footers outside the scroll body with `flex_none()`.
- Avoid manually estimating `Scrollbar::scroll_size(...)` for ordinary dynamic `v_flex` content. If the estimate drifts from the real content bounds, the scrollbar can appear full or move independently from the target content.

...
name: serial
description: >-
  嵌入式串口调试工具，用于扫描串口、实时监控、发送数据、记录日志、Hex 查看和 TCP 透明转发。
  当用户提到串口、COM 口、UART、AT 命令调试、波特率、Hex 串流、串口抓日志、
  串口监控、查看 MCU 输出、二进制协议联调、将串口转成 TCP 时自动触发，也兼容 /serial 显式调用。
...
统一封装端口发现、实时监控、数据发送、日志记录、Hex 查看和 TCP 透明转发能力。
...
| `hex` | 实时查看二进制流 | 低 |
| `log` | 保存串口日志到文件 | 低 |
| `tcp-forward` | 将本地串口透明转发为 TCP 字节流 | 中 |
...
除 `serial_tcp_forward.py` 这种显式端口转发工具外，脚本会按优先级从 CLI 参数、工程级配置、状态文件中读取参数。
...
# 串口透明转 TCP 字节流
python scripts/serial_tcp_forward.py --serial <串口> --baud <波特率> --listen <ip:port>
```
...
当目标开发板连接在远端机器上，而本机的 CLI/TUI/GPUI 需要通过 `--tcp <host:port>` 访问时，使用 `serial_tcp_forward.py` 在远端机器上独占打开真实串口，并监听一个 TCP 端口进行双向透明转发。

### 使用示例

```bash
# 远端机器：开发板 CDC/UART 插在这里
python skills/serial/scripts/serial_tcp_forward.py \
  --serial /dev/ttyACM0 \
  --baud 115200 \
  --listen 0.0.0.0:5657

# 本机：通过 rseq TCP transport 连接
...
| 参数 | 说明 |
|------|------|
| `--serial` | 真实串口路径，如 `/dev/ttyACM0`、`/dev/cu.usbmodem314201`、`COM3` |
| `--listen` | TCP 监听地址，如 `127.0.0.1:5657`、`0.0.0.0:5657`、`[::1]:5657` |
| `--baud` | 串口波特率，默认 `115200` |
| `--clear` | 启动时清空串口输入/输出缓冲；默认不清，避免截断正在上报的 report |
| `--replace-client` | 单客户端模式下，新客户端连接时关闭旧客户端 |
| `--multi-client` | 将串口 RX 广播给多个 TCP 客户端；多个客户端同时写入会导致串口帧交错 |

### 行为规则

- 默认单客户端，额外客户端会被拒绝，以避免多个工具同时写串口造成 rseq 帧混乱。
- 需要切换客户端时优先使用 `--replace-client`，而不是 `--multi-client`。
- 只观察日志/report 且明确知道没有客户端会写入时，才使用 `--multi-client`。
- `--listen 0.0.0.0:PORT` 会暴露给局域网；仅本机测试时使用 `127.0.0.1:PORT`。
- 如果缺少依赖，安装 `pyserial`：`python3 -m pip install pyserial`。
